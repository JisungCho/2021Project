	...
	<div class="container m-5 p-2 rounded mx-auto bg-light shadow">
			...
			<div class="col col-11 mx-auto">
				<div class="row bg-white rounded shadow-sm p-2 add-todo-wrapper align-items-center justify-content-center">
					...
					<div class="col-auto m-0 px-2 d-flex align-items-center">
						<label class="text-secondary my-2 p-0 px-1 view-opt-label due-date-label d-none"></label> <i class="fa fa-calendar my-2 px-1 text-primary btn due-date-button" data-toggle="tooltip" data-placement="bottom" title="Set a Due date"></i>
					</div>
		...
		<div class="row m-1 p-3 px-5 justify-content-end">
			<div class="col-auto d-flex align-items-center">
				<label class="text-secondary my-2 pr-2 view-opt-label">Filter</label> <select id="select" class="custom-select custom-select-sm btn my-2">
				...
				<c:forEach items="${todolist}" var="todo">
					<div class="row px-3 align-items-center todo-item rounded">
						...
						<div class="col px-1 m-1 d-flex align-items-center">
							<!-- 상태가 ACTIVE 이거나 HAS_DUE_DATE인 경우 체크표시 없음 -->
							<c:if test="${todo.todo_state == 'ACTIVE' || todo.todo_state == 'HAS_DUE_DATE' }">
							...
							<c:if test="${todo.todo_date != null }">
								<div class="row">
									<div class="col-auto d-flex align-items-center rounded bg-white border border-warning">
										<c:set var="date">
											<fmt:formatDate value="${todo.todo_date}" pattern="yyyy/MM/dd" />
										</c:set>
										<c:choose>
											<c:when test="${todo.todo_state == 'HAS_DUE_DATE' }">
												<h6 id="todo_label${todo.seq }" class="text my-2 pr-2 text-danger font-italic font-weight-bold">
													<fmt:formatDate value="${todo.todo_date }" pattern="yyyy/MM/dd" />
												</h6>
											</c:when>
											<c:when test="${todo.todo_state != 'HAS_DUE_DATE'}">
												<h6 id="todo_label${todo.seq }" class="text my-2 pr-2 font-italic">
													<fmt:formatDate value="${todo.todo_date }" pattern="yyyy/MM/dd" />
												</h6>
											</c:when>
										</c:choose>
						...
						<div class="col-auto m-1 p-0 todo-actions">
							...
							<!--  상태가 ACTIVE이거나 HAS_DUE_DATE인 경우 수정가능 -->
								<c:if test="${todo.todo_state == 'ACTIVE' || todo.todo_state == 'HAS_DUE_DATE' }">
							...
							<div class="row todo-created-info">
								<div class="col-auto d-flex align-items-center pr-2">
									<i class="fa fa-info-circle my-2 px-2 text-black-50 btn" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Created date"></i> <label class="date-label my-2 text-black-50"> <fmt:formatDate value="${todo.reg_date }" pattern="yyyy/MM/dd" />
									</label>
								</div>
							</div>
						</div>
